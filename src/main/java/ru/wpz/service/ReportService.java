...
public class ReportService {
    ...
    public ReportMomentDto findDevOrg(long id) {
        ...
        if (organization != null){
            deviceList = getAllDevice(organization);
        }
    ...
    public DayReportDto findDayDevOrg(long id, LocalDateTime day) {
        ...
        Organization organization = organizationRepository.findById(id).orElse(null);
        reportList = getReportOnOrg(organization, start, end);
    ...
    public ReportPeriodDto findPeriod(long id,LocalDateTime start, LocalDateTime end){
        ...
        Organization organization = organizationRepository.findById(id).orElse(null);
        reportList = getReportOnOrg(organization, start, end);
        return findReportPeriod(reportList, start, end);
    }
    ...
    private ReportPeriodDto findReportPeriod(List<Report> listR, LocalDateTime start, LocalDateTime end){
        boolean flag = false;
        ...
        reportPeriodDto.setTotal(countingTime.size());
        reportPeriodDto.setPercent((double) Math.floorDiv(reportPeriodDto.getTotal() * 60, minute));
